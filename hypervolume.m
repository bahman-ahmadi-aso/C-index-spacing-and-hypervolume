...
function Score = hypervolume(PS,R)  %hypervolume

% Normalize the population according to the reference point set
[N,M]  = size(PS);
fmin   = min(min(PS,[],1),zeros(1,M));
fmax   = max(R,[],1);
PS = (PS-repmat(fmin,N,1))./repmat((fmax-fmin)*1.1,N,1);
PS(any(PS>1,2),:) = [];
